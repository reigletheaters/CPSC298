...
void Game::initializeGrid()
{
  ...
  for (int iRow = 0; iRow < m_nGridRows; ++iRow){ /* ?? complete for loop ?? */
    for (int iColumn = 0;  iColumn < m_nGridColumns; ++iColumn){/* ?? complete for loop ?? */
...
void Game::displayGrid(bool bShowBoat) {
  ...
  for (int iRow = 0; iRow < m_nGridRows; ++iRow){ /* ?? complete for loop specification ?? */

    std::cout << "|";
    for (int iColumn = 0; iColumn < m_nGridColumns; ++iColumn){ /* ?? complete for loop specification ?? */
...
void Game::play()
{
  ...
  while ((!bSunk) && (this->m_nTorpedoes > 0))
  {
    ...
    if (!bSunk)
    {
      std::cout << "Magazine: " << m_nTorpedoes /* ?? remaining number of torpedoes ?? */ << " torpedoes" << std::endl;
    }
